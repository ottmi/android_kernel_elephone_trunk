...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	if (pre_pen && (touch_num == 0)) {
		dev_err(&ts->client->dev, "Pen touch UP(Slot)!");
...
static s8 gtp_enter_sleep(struct goodix_ts_data  *ts)
{
	...
	if (ts->pdata->enable_power_off) {
		...
		while (retry++ < 5) {
			ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
			if (ret == 1) {
				dev_err(&ts->client->dev, "GTP enter sleep!");
				return ret;
			}
			msleep(20);
		}
...
err_retry:
		...
		if (ret == 2) {
			dev_err(&ts->client->dev, "GTP wakeup sleep.");
#if (!GTP_SLIDE_WAKEUP)
			if (chip_gt9xxs == 0) {
				gtp_int_sync(ts, 25);
				msleep(20);
...
static int gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++)
		dev_err(&client->dev, "Config Groups(%d) Lengths: %d",
...
Output:
	...
	> =0: succeed, otherwise: failed
*******************************************************/
static int unisope_init_panel(struct goodix_ts_data *ts)
{
	struct i2c_client *client = ts->client;
	u8 config_data[5];
	int ret = -EIO;

	/*read X_max Low 8 Bit*/
	ret = gtp_i2c_read_dbl_check(ts->client, (GTP_REG_CONFIG_DATA+1),
			&config_data[0], 1);
	if(ret < 0)
	...
	/*read Y_max Low 8 Bit*/
	ret = gtp_i2c_read_dbl_check(ts->client, (GTP_REG_CONFIG_DATA+3),
			&config_data[2], 1);
	if(ret < 0)
	dev_err(&client->dev, "read Y_max Low 8 Bit failed");
	/*read Y_max High 8 Bit*/
	ret = gtp_i2c_read_dbl_check(ts->client, (GTP_REG_CONFIG_DATA+4),
			&config_data[3], 1);
	if(ret < 0)
	dev_err(&client->dev, "read Y_max High 8 Bit failed");
      ts->abs_y_max = (config_data[3] << 8) | config_data[2];
	dev_err(&client->dev, "read Y_max  %d",ts->abs_y_max);

	ret = gtp_i2c_read_dbl_check(ts->client, (GTP_REG_CONFIG_DATA+6),
			&config_data[4], 1);
	if(ret < 0)
...
#else
	ret = unisope_init_panel(ts);
	if (ret < 0) {
		dev_err(&client->dev, "GTP init panel failed.\n");
		ts->abs_x_max = pdata->x_max;
		ts->abs_y_max = pdata->y_max;
		ts->int_trigger_type = GTP_INT_TRIGGER;
	}
#endif
...
void gtp_esd_switch(struct i2c_client *client, int on)
{
	...
	if (SWITCH_ON == on) {
		...
		if (!ts->esd_running) {
			ts->esd_running = 1;
			dev_err(&client->dev, "Esd started\n");
		...
		if (ts->esd_running) {
			ts->esd_running = 0;
			dev_err(&client->dev, "Esd cancelled\n");
			cancel_delayed_work_sync(&gtp_esd_check_work);
		}
	}
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if (ts->gtp_is_suspend) {
		dev_err(&ts->client->dev, "Esd terminated!\n");
		ts->esd_running = 0;
		return;
	}
#ifdef CONFIG_GT9XX_TOUCHPANEL_UPDATE
