 ...
 *  Copyright (C) 2012-2014 Lex Hsieh / sensortek <lex_hsieh@sitronix.com.tw> or
...
static void stk_init_code_threshold_table(struct stk3x1x_data *ps_data)
{
	...
	dev_dbg(&ps_data->client->dev, "alscode[0]=%d\n", 0);
#endif
		...
		struct stk3x1x_platform_data *plat_data)
{
	...
	if (ret < 0) {
		dev_dbg(&ps_data->client->dev, "%s: write i2c error\n",
				__func__);
		return ret;
	}
	...
	ret = i2c_smbus_write_byte_data(ps_data->client,
			STK_ALSCTRL_REG, w_reg);
	if (ret < 0) {
		dev_dbg(&ps_data->client->dev, "%s: write i2c error\n",
				__func__);
		return ret;
...
#if (!defined(STK_POLL_ALS)) && (STK_INT_PS_MODE != 0x02) \
	&& (STK_INT_PS_MODE != 0x03)
	w_reg |= STK_INT_ALS;
#endif
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	if (ret < 0) {
		dev_err(&ps_data->client->dev, "%s: write i2c error, ret=%d\n",
			__func__, ret);
		return ret;
	}
	w_state_reg = ret;
	...
	if (enable) {
		...
		if (ret < 0) {
			dev_err(&ps_data->client->dev, "%s: read i2c error, ret=%d\n",
				__func__, ret);
			return ret;
		}
...
static int32_t stk3x1x_get_ir_reading(struct stk3x1x_data *ps_data)
{
	...
	tmp_word_data = i2c_smbus_read_word_data(ps_data->client,
		STK_DATA1_IR_REG);
	if (tmp_word_data < 0) {
		dev_dbg(&ps_data->client->dev, "%s fail, err=0x%x",
			__func__, tmp_word_data);
		return tmp_word_data;
	}
	word_data = ((tmp_word_data & 0xFF00) >> 8) |
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		dev_err(&ps_data->client->dev, "%s, invalid value %d\n",
		__func__, *buf);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret < 0) {
		dev_err(&ps_data->client->dev, "%s:kstrtoul failed, ret=0x%x\n",
			__func__, ret);
		return ret;
	}
	mutex_lock(&ps_data->io_lock);
	...
	struct device_attribute *attr, char *buf)
{
	...
	tmp_word_data = i2c_smbus_read_word_data(ps_data->client,
		STK_DATA1_OFFSET_REG);
	if (tmp_word_data < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, tmp_word_data);
		return tmp_word_data;
	}
	word_data = ((tmp_word_data & 0xFF00) >> 8) |
		((tmp_word_data & 0x00FF) << 8);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value > 65535) {
		dev_err(dev, "%s: invalid value, offset=%ld\n",
			__func__, value);
		return -EINVAL;
	}

	offset = (uint16_t) ((value&0x00FF) << 8) | ((value&0xFF00) >> 8);
	ret = i2c_smbus_write_word_data(ps_data->client,
		STK_DATA1_OFFSET_REG, offset);
	if (ret < 0) {
		dev_err(dev, "%s: write i2c error\n",
			__func__);
		return ret;
	}
	return size;
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = stk3x1x_get_flag(ps_data);
	if (ret < 0) {
		dev_dbg(dev, "%s: stk3x1x_get_flag failed, ret=0x%x\n",
			__func__, ret);
		return ret;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	mutex_lock(&ps_data->io_lock);
	ps_thd_l1_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDL1_PS_REG);
	if (ps_thd_l1_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, ps_thd_l1_reg);
		return -EINVAL;
	}
	ps_thd_l2_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDL2_PS_REG);
	if (ps_thd_l2_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, ps_thd_l2_reg);
		return -EINVAL;
	}
	mutex_unlock(&ps_data->io_lock);
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	mutex_lock(&ps_data->io_lock);
	ps_thd_h1_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDH1_PS_REG);
	if (ps_thd_h1_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, ps_thd_h1_reg);
		return -EINVAL;
	}
	ps_thd_h2_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDH2_PS_REG);
	if (ps_thd_h2_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, ps_thd_h2_reg);
		return -EINVAL;
	}
	mutex_unlock(&ps_data->io_lock);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (cnt = 0; cnt < 25; cnt++) {
		...
		if (ps_reg[cnt] < 0) {
			mutex_unlock(&ps_data->io_lock);
			dev_err(dev,
				"stk_all_reg_show:i2c_smbus_read_byte_data fail, ret=%d",
				ps_reg[cnt]);
			return -EINVAL;
		} else {
	...
	if (ps_reg[cnt] < 0) {
		mutex_unlock(&ps_data->io_lock);
		dev_err(dev, "all_reg_show:i2c_smbus_read_byte_data fail, ret=%d",
			ps_reg[cnt]);
		return -EINVAL;
	}
	...
	return scnprintf(buf, PAGE_SIZE,
		...
		ps_reg[4], ps_reg[5], ps_reg[6], ps_reg[7],
		ps_reg[8], ps_reg[9], ps_reg[10], ps_reg[11],
		ps_reg[12], ps_reg[13], ps_reg[14], ps_reg[15],
		ps_reg[16], ps_reg[17], ps_reg[18], ps_reg[19],
		ps_reg[20], ps_reg[21], ps_reg[22], ps_reg[23],
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (0 != ret) {
		dev_err(dev,
			"%s: i2c_smbus_write_byte_data fail\n", __func__);
		return ret;
	}
...
{
	struct stk3x1x_data *ps_data = container_of(timer,
		struct stk3x1x_data, als_timer);
...
static void stk_ps_work_func(struct work_struct *work)
{
	...
	/* read 0x10 flag reg */
	if (org_flag_reg < 0) {
		dev_err(&ps_data->client->dev, "%s: get_status_reg fail, ret=%d",
			__func__, org_flag_reg);
		goto err_i2c_rw;
	}
	near_far_state = (org_flag_reg & STK_FLG_NF_MASK) ? 1 : 0;
	/* STK_FLG_NF_MASK = 0x01 */
	reading = stk3x1x_get_ps_reading(ps_data);
	...
	if (ret < 0) {
		dev_err(&ps_data->client->dev, "%s:stk3x1x_set_flag fail, ret=%d\n",
			__func__, ret);
		goto err_i2c_rw;
	}
...
static void stk_work_func(struct work_struct *work)
{
	...
	reading = stk3x1x_get_ps_reading(ps_data);
#ifdef STK_DEBUG_PRINTF
	dev_dbg(&ps_data->client->dev, "%s: ps input event %d cm, ps code = %d\n",
		__func__, near_far_state, reading);
#endif
#else
	/* mode 0x01 or 0x04 */
	...
	if (org_flag_reg < 0) {
		dev_err(&ps_data->client->dev,
			"%s: get_status_reg fail, org_flag_reg=%d",
			__func__, org_flag_reg);
		goto err_i2c_rw;
	}
...
#endif /* CONFIG_STK_PS_ALS_USE_CHANGE_THRESHOLD */
		ps_data->als_lux_last = stk_alscode2lux(ps_data, reading);
		input_report_abs(ps_data->als_input_dev,
			ABS_MISC, ps_data->als_lux_last);
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF
		dev_err(&ps_data->client->dev,
			"%s: als input event %d lux\n",
...
static int stk3x1x_setup_irq(struct i2c_client *client)
{
...
#ifdef STK_DEBUG_PRINTF
	dev_dbg(&ps_data->client->dev, "%s: int pin #=%d, irq=%d\n",
	...
	if (err < 0) {
		dev_err(&ps_data->client->dev, "%s: gpio_request, err=%d",
			__func__, err);
		return err;
	}
	...
	err = request_any_context_irq(irq, stk_oss_irq_handler,
		IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, DEVICE_NAME, ps_data);
#else
	err = request_any_context_irq(irq, stk_oss_irq_handler,
...
{
	struct stk3x1x_data *ps_data =
		container_of(h, struct stk3x1x_data, stk_early_suspend);
#ifndef STK_POLL_PS
	int err;
#endif
	...
	const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter,
		...
		dev_err(&client->dev,
			"%s: No Support for I2C_FUNC_SMBUS_BYTE_DATA\n",
			__func__);
		return -ENODEV;
	}
	if (!i2c_check_functionality(client->adapter,
		...
		dev_err(&client->dev,
			"%s: No Support for I2C_FUNC_SMBUS_WORD_DATA\n",
			__func__);
		return -ENODEV;
	}
	...
	if (!ps_data) {
		dev_err(&client->dev,
			"%s: failed to allocate stk3x1x_data\n", __func__);
		return -ENOMEM;
	}
	...
	if (ps_data->als_input_dev == NULL) {
		dev_err(&client->dev,
			"%s: could not allocate als device\n",
			__func__);
		err = -ENOMEM;
		goto err_als_input_allocate;
	}
	...
	if (ps_data->ps_input_dev == NULL) {
		dev_err(&client->dev,
			"%s: could not allocate ps device\n",
			__func__);
		err = -ENOMEM;
	...
	if (err < 0) {
		dev_err(&client->dev,
			"%s: can not register ps input device\n", __func__);
